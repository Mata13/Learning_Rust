...
// ---------- MATRIZ DEL SISTEMA dx/dt = -3x + 2y, dy/dt = 6x - 4y ----------
const A11: f64 = -3.0;
const A12: f64 =  2.0;
const A21: f64 =  6.0;
const A22: f64 = -4.0;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let nombre_archivo = "Tarea03_06_f.png";

    let root = BitMapBackend::new(nombre_archivo, (900, 900)).into_drawing_area();
    root.fill(&WHITE)?;

    let mut chart = ChartBuilder::on(&root)
        .caption("Retrato de Fase — dx/dt = -3x+2y, dy/dt = 6x-4y", ("sans-serif", 22))
        .margin(15)
    ...
    for &x0 in &[-2.0, -1.0, 0.0, 1.0, 2.0] {
        for &y0 in &[-2.0, -1.0, 0.0, 1.0, 2.0] {
            ...
            for _ in 0..pasos {
                ...
                cx = nx;
                cy = ny;
                // por si alguna trayectoria se dispara en otra configuración
